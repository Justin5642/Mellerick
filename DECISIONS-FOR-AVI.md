...
| D10 | Lint: the codebase uses `any` deliberately for external/loosely-typed shapes (Supabase joins, Xero/Google SDK errors) — 271 `no-explicit-any` warnings. Rather than a risky mass-rewrite, set that rule to `warn` and enabled lint in builds (0 errors). Properly typing these is a separate initiative | Approve warn-vs-error, or schedule a typing pass |
| D11 | `react/no-unescaped-entities` disabled — false positive on react-pdf `<Text>` (renders to PDF, not DOM) | FYI |
| D12 | Mobile: single pre-existing tsc error (`collapsable` prop type gap on ImageBackground) fixed with a `@ts-expect-error` (zero runtime change — the prop is a needed capture workaround). Mobile app not otherwise touched; `expo-doctor` deferred (heavy, out of scope) | FYI |
| D13 | Mobile `AGENTS.md` says "read Expo v57 docs" but `mobile/package.json` pins Expo SDK 54 — a doc/version inconsistency, not fixed here | Confirm intended Expo version |
