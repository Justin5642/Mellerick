...
- Q10: A sync-status pill (pending count / "N failed") is wired in `useSyncStatus` (now reports terminal `dead` writes) but not yet placed in any screen header. Where should it live — global header, or per-job-detail? (Purely additive; no correctness impact.)
