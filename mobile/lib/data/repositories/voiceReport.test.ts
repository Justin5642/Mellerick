...
describe("VoiceReportRepository", () => {
  it("record enqueues an upload-only op then a transcribe side-effect that depends on it", async () => {
    ...
    expect(storagePath).toBe("j1/voice-report-id-1.m4a");
    ...
    expect(upload.payload).toEqual({ bucket: "job-audio", storage_path: "j1/voice-report-id-1.m4a" });
    ...
    expect(transcribe.payload).toEqual({ jobId: "j1", storagePath: "j1/voice-report-id-1.m4a", recordedBy: "u1" });
