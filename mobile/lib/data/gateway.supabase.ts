...
export const supabaseGateway: SupabaseGateway = {
  ...
  async deleteRow(table, id) {
    ...
    //
    // Migration 0049 (applied 2026-08-22) makes job_photos deletable only by
    // office/admin or the assigned technician, which makes the RLS-refusal
    // case reachable — everything above here treats a resolved promise as done
  ...
  async removeObject(bucket, path) {
    // Best-effort, never throws — correct because the ROW delete is the gate.
    // deleteRow throws when RLS refuses, so an unauthorized photo delete
    // (migration 0049, applied 2026-08-22) fails there, loudly, before this
    // ever runs; both are scoped by the same predicate, so an authorized
    // caller cannot be refused here. What
