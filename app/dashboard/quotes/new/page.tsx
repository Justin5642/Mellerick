...
export default function NewQuotePage() {
  ...
  useEffect(() => {
    async function loadJobs() {
      ...
      const { data } = await supabase.from("jobs").select("id, job_number, title").eq("customer_id", form.customer_id);
      setJobs(data ?? []);
    }
    loadJobs();
  }, [form.customer_id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error || !quote) { toast.error(error?.message ?? "Failed to create quote"); setLoading(false); return; }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Notes</Label>
              <Textarea value={form.notes} onChange={e => setField("notes", e.target.value)} placeholder="Terms, exclusions, notes..." rows={2} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-3">
          <Link href="/dashboard/quotes"><Button variant="outline" type="button">Cancel</Button></Link>
          <Button type="submit" disabled={loading}>{loading ? "Saving..." : "Create Quote"}</Button>
        </div>
      </form>
    </div>
  );
