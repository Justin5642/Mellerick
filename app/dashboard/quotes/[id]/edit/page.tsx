...
export default function EditQuotePage() {
  ...
  const [form, setForm] = useState({ title: "", customer_id: "", customer_name: "", valid_until: "", notes: "" });
  const [lineItems, setLineItems] = useState<LineItem[]>([]);
  const [errors, setErrors] = useState<Record<string, boolean>>({});

  useEffect(() => {
    async function load() {
      const [{ data: quote }, { data: pricing }] = await Promise.all([
      ...
      if (quote) {
        ...
        setLineItems((quote.quote_items ?? []).map((i: any) => ({
          id: i.id,
          name: i.name,
    ...
    <div className="p-6 max-w-3xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3">
          <Link href={`/dashboard/quotes/${id}`}><Button variant="outline" type="button">Cancel</Button></Link>
