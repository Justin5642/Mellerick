...
export async function POST(request: NextRequest) {
  try {
    ...
    // Server-side enforcement (not just a UI hint): only admins may push invoices
    // to Xero, whether triggered from the Approvals auto-push or the manual button.
    // Per Justin: technicians must never push, including by calling this API
    // directly. requireAdmin resolves the caller from a Bearer token (mobile) OR a
    // cookie (web); callerClient scopes the DB read to that admin under RLS.
